...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D_label(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
        ...
        with context():
            with torch.no_grad():
                if self.conv_op == nn.Conv3d:
                    res = self._internal_predict_3D_3Dconv_label(x, patch_size, do_mirroring, mirror_axes, regions_class_order,
                                                               pad_border_mode, pad_kwargs=pad_kwargs, verbose=verbose, classification=classification)
                else:
    ...
    def _internal_predict_3D_3Dconv_tiled_label(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          patch_size: tuple, regions_class_order: tuple, use_gaussian: bool,
                                          pad_border_mode: str, pad_kwargs: dict, all_in_gpu: bool,
                                          verbose: bool, classification: bool):
...
#                     _internal_predict_3D_3Dconv_tiled_label: classification
...
#         """)
    ...
    def _internal_predict_3D_3Dconv_label(self, x: np.ndarray, min_size: Tuple[int, ...], do_mirroring: bool,
                                    mirror_axes: tuple = (0, 1, 2), regions_class_order: tuple = None,
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    verbose: bool = True, classification: bool = True):
        """
        This one does fully convolutional inference. No sliding window
        """
...
#                     _internal_predict_3D_3Dconv_label: classification
#                     data: {data.shape} ({type(data)})
#                     data[None]: {data[None].shape} ({type(data[None])})
...
#                     data: {data.shape} ({type(data)})
#                     data[None]: {data[None].shape} ({type(data[None])})
#                     prediction: {class_pred.argmax(1)}
                
#                 ###############################################################
        
#         """)
        
#         predicted_probabilities = self._internal_maybe_mirror_and_pred_3D_label(data[None], mirror_axes, do_mirroring,
#                                                                           None)[0]
...
#         return predicted_segmentation, predicted_probabilities
    ...
    def _internal_maybe_mirror_and_pred_3D_label(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
