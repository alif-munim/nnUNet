...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        # NEW
        self.online_eval_classification_accuracy = []
...
#             output_softmax = softmax_helper(output_seg)
#             output_seg = output_softmax.argmax(1)
#             target_seg = target_seg[:, 0]
#             axes = tuple(range(1, len(target_seg.shape)))
#             tp_hard = torch.zeros((target_seg.shape[0], num_classes - 1)).to(output_seg.device.index)
#             fp_hard = torch.zeros((target_seg.shape[0], num_classes - 1)).to(output_seg.device.index)
#             fn_hard = torch.zeros((target_seg.shape[0], num_classes - 1)).to(output_seg.device.index)
#             for c in range(1, num_classes):
#                 tp_hard[:, c - 1] = sum_tensor((output_seg == c).float() * (target_seg == c).float(), axes=axes)
#                 fp_hard[:, c - 1] = sum_tensor((output_seg == c).float() * (target_seg != c).float(), axes=axes)
#                 fn_hard[:, c - 1] = sum_tensor((output_seg != c).float() * (target_seg == c).float(), axes=axes)
...
#             self.online_eval_classification_accuracy.append(accuracy.item())
...
#         self.online_eval_classification_accuracy = np.mean(self.online_eval_classification_accuracy)
#         self.print_to_log_file("Average classification accuracy:", np.round(self.online_eval_classification_accuracy, 4))
...
#             # Handle seg_output when it's a tuple (due to deep supervision)
...
#             num_classes = seg_output.shape[1]
            ...
            # print(f"run_online_evaluation: Shape of seg_output: {seg_output[0].shape}")
            # print(f"run_online_evaluation: Shape of class_output: {class_output.shape}")
            # print(f"run_online_evaluation: Shape of seg_target: {seg_target[0].shape}")
            # print(f"run_online_evaluation: Shape of class_target: {class_target.shape}")
            
#             # CONVERT SEG_TARGET TO A TENSOR IF IT'S A LIST
#             # CONVERT SEG_TARGET AND CLASS_TARGET TO TENSORS IF THEY ARE LISTS
#             # if isinstance(seg_target, list):
#             #     seg_target = torch.tensor(seg_target).to(seg_output.device)
#             # if isinstance(class_target, list):
#             #     class_target = torch.tensor(class_target).to(class_output.device)

#             output_softmax = softmax_helper(seg_output)
#             seg_output = output_softmax.argmax(1)
#             seg_target = seg_target[:, 0]
#             axes = tuple(range(1, len(seg_target.shape)))
#             tp_hard = torch.zeros((seg_target.shape[0], num_classes - 1)).to(seg_output.device.index)
#             fp_hard = torch.zeros((seg_target.shape[0], num_classes - 1)).to(seg_output.device.index)
#             fn_hard = torch.zeros((seg_target.shape[0], num_classes - 1)).to(seg_output.device.index)
#             for c in range(1, num_classes):
#                 tp_hard[:, c - 1] = sum_tensor((seg_output == c).float() * (seg_target == c).float(), axes=axes)
#                 fp_hard[:, c - 1] = sum_tensor((seg_output == c).float() * (seg_target != c).float(), axes=axes)
#                 fn_hard[:, c - 1] = sum_tensor((seg_output != c).float() * (seg_target == c).float(), axes=axes)
...
#             self.online_eval_fn.append(list(fn_hard))
            ...
            # pred_class = torch.argmax(class_output, dim=1)
            # correct = (pred_class == class_target).float().sum()
            # print(f"pred_class: {pred_class.shape}")
            # print(f"class_target: {class_target.shape}")
            # print(f"Correct classifications this batch: {correct.item()}")
            # print(f"Total classifications this batch: {class_target.size(0)}")
